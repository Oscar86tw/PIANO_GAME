...
(function(){
  ...
  async function load(){
    ...
    for(const n of sampleNotes){
      try{
        ...
        buffers.set(noteToMidi(n),await c.decodeAudioData(ab));ok++;
      }catch(e){}
    }
    ready=ok>0;
  ...
  async function play(note,velocity=.8,delaySec=0,duration=.8){
    ...
    const start=c.currentTime+Math.max(0,Number(delaySec)||0),dur=Math.max(.08,Number(duration)||.8);
    gain.gain.setValueAtTime(Math.max(.04,Math.min(1,velocity)),start);
    gain.gain.exponentialRampToValueAtTime(.001,start+dur);
    source.connect(gain).connect(c.destination);source.start(start);source.stop(start+dur+.08);
    return {source,gain,start,note};
  }

  async function click(accent=false,delaySec=0){
    const c=await init(),osc=c.createOscillator(),gain=c.createGain();
    const start=c.currentTime+Math.max(0,Number(delaySec)||0);
    osc.type='sine';osc.frequency.setValueAtTime(accent?1450:1050,start);
    gain.gain.setValueAtTime(accent?.12:.07,start);gain.gain.exponentialRampToValueAtTime(.001,start+.055);
    osc.connect(gain).connect(c.destination);osc.start(start);osc.stop(start+.065);
    return {source:osc,gain,start};
  }

  function stopHandle(h){
    try{h?.source?.stop?.()}catch(e){}
  }

  window.AudioEngine={
    init,load,play,click,stopHandle,
