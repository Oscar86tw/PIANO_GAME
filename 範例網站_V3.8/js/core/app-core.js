...
let scoreRepairInProgress=false;
const songs = {
  twinkle:{
...
// ---------- V3.8: 216 built-in pedagogical scores ----------
...
// ---------- V3.8: full-length repertoire collections ----------
...
// ---------- V3.8: generated left-hand accompaniment ----------
...
document.querySelectorAll('.tab').forEach(btn=>{
  btn.addEventListener('click',()=>{
    if(window.MODULAR_NAVIGATION_ACTIVE)return;
...
function openPractice(songId,label='PLAY'){
  if(window.PAGE_MODULE && window.PAGE_MODULE!=='practice' && window.MODULE_ROUTES?.practice){
    try{
      sessionStorage.setItem('pianoPendingPractice',JSON.stringify({songId,label}));
    }catch(e){}
    const u=new URL(window.MODULE_ROUTES.practice,location.href);
...
$('scoreHealthBtn').addEventListener('click',()=>{
  const ok=ensureScoreVisible('使用者手動檢查',true);
  if(ok){
    $('transportStatus').textContent='樂譜顯示正常';
  }
});
...
$('soundPackFilesInput').addEventListener('change',async e=>{
  ...
  try{
    ...
    reportAppError('音色包',err,{title:'音色包匯入失敗'});
  }
  e.target.value='';
});
...
function sampleFileFor(noteName){
  return (window.APP_BASE||'')+'assets/piano/'+noteName.replace('#','s')+'.mp3';
}
...
async function loadLocalPianoSamples(){
  ...
  try{
    ...
    reportAppError('鋼琴音色',err,{title:'鋼琴 sample 載入失敗'});
    return false;
  }finally{
...
function noteY(note){
  ...
  const base=4*7+degree.E;
  const y=138-(step-base)*7.5;
  return Math.max(22,Math.min(256,y));
}
...
function bassNoteY(note){
  ...
  if(!m)return 112;
  const step=(+m[2])*7+degree[m[1]];
  const base=2*7+degree.G;
  const y=146-(step-base)*6.5;
  return Math.max(22,Math.min(256,y));
}
...
function renderStaffTimeline(root,timeline,hand){
  if(!root)throw new Error(`找不到 ${hand} scrolling score 容器`);
  root.innerHTML='';
  const song=songs[state.song];
  if(!song)throw new Error(`找不到曲目資料：${state.song}`);
  if(!Array.isArray(timeline))throw new Error(`${hand} timeline 不是陣列`);
  ...
  timeline.forEach(ev=>{
    ...
    if(ev.isRest){
      ...
      notes.forEach((note,chordIndex)=>{
        ...
        n.style.left=x+'px';
        n.style.top=(hand==='left'?bassNoteY(note):noteY(note))+'px';
        n.style.opacity='1';
        n.style.visibility='visible';
        n.title=`${hand==='left'?'左手':'右手'} ${note}`;
  ...
  const playhead=(targetWindow?.clientWidth||800)*.22;
  const initialX=playhead-lead;
  root.style.transform=`translate3d(${Number.isFinite(initialX)?initialX:0}px,0,0)`;
}
function expectedVisibleNoteCount(timeline){
  return (timeline||[]).filter(e=>!e.isRest).reduce((sum,e)=>sum+(Array.isArray(e.note)?e.note.length:1),0);
}
function scoreDomHealth(){
  const rightExpected=expectedVisibleNoteCount(state.rightTimeline);
  const leftExpected=expectedVisibleNoteCount(state.leftTimeline);
  const rightDom=$('scrollingScore')?.querySelectorAll('.music-note').length||0;
  const leftDom=$('scrollingBassScore')?.querySelectorAll('.music-note').length||0;
  return {rightExpected,leftExpected,rightDom,leftDom,
    ok:(rightExpected===0||rightDom>0)&&(leftExpected===0||leftDom>0)};
}
function updateScoreHealthUI(health){
  if(!$('scoreHealthBtn'))return;
  $('scoreHealthBtn').classList.remove('is-warning','is-error');
  if(!health){
    $('scoreHealthText').textContent='檢查中';
    $('scoreHealthBtn').classList.add('is-warning');
    return;
  }
  if(health.ok){
    $('scoreHealthText').textContent=`正常 ${health.rightDom}+${health.leftDom}`;
  }else{
    $('scoreHealthText').textContent='需修復';
    $('scoreHealthBtn').classList.add('is-error');
  }
}
function ensureScoreVisible(reason='自動檢查',allowRepair=true){
  let health=scoreDomHealth();
  updateScoreHealthUI(health);
  if(health.ok)return true;
  if(!allowRepair||scoreRepairInProgress)return false;

  scoreRepairInProgress=true;
  try{
    buildEventTimeline();
    ...
    updateHandFocus();
    health=scoreDomHealth();
    updateScoreHealthUI(health);
    if(!health.ok){
      reportAppError('樂譜顯示',
        `時間軸有音符，但畫面音符數量異常。右手 ${health.rightDom}/${health.rightExpected}，左手 ${health.leftDom}/${health.leftExpected}`,
        {title:'樂譜音符沒有顯示',detail:'reason='+reason});
    }
    return health.ok;
  }catch(err){
    updateScoreHealthUI({ok:false,rightDom:0,leftDom:0});
    reportAppError('樂譜顯示',err,{title:'重新建立樂譜失敗',detail:'reason='+reason});
    return false;
  }finally{
    scoreRepairInProgress=false;
  }
}
function renderStaticScore(){
  updateScoreHealthUI(null);
  try{
    buildEventTimeline();
    renderStaffTimeline($('scrollingScore'),state.rightTimeline,'right');
    renderStaffTimeline($('scrollingBassScore'),state.leftTimeline,'left');
    updateHandFocus();
    requestAnimationFrame(()=>ensureScoreVisible('renderStaticScore',true));
  }catch(err){
    reportAppError('樂譜顯示',err,{title:'建立樂譜失敗'});
  }
}
...
function gameLoop(){
  ...
  updateMasterBeat(e);
  let pxPerBeat=+root?.dataset?.pxPerBeat;
  let lead=+root?.dataset?.lead;
  const staff=$('staffWindow');
  const playhead=(staff?.clientWidth||800)*.22;
  if(!Number.isFinite(pxPerBeat)||pxPerBeat<=0||!Number.isFinite(lead)){
    ensureScoreVisible('gameLoop 缺少譜面尺寸資料',true);
    pxPerBeat=+root?.dataset?.pxPerBeat||95;
    lead=+root?.dataset?.lead||(window.innerWidth<600?260:380);
  }
  const secPerBeat=beatSeconds();
  const x=playhead-lead-(Number.isFinite(e)&&secPerBeat>0?(e/secPerBeat)*pxPerBeat:0);
  if(Number.isFinite(x)){
    root.style.transform=`translate3d(${x}px,0,0)`;
    if(bassRoot) bassRoot.style.transform=`translate3d(${x}px,0,0)`;
  }
...
function flash(kind){
  // V3.8: intentionally disabled. No screen flash at beat/hit time.
}
function showAssist(){
...
async function connectMIDI(){
  if(!navigator.requestMIDIAccess){
    ...
    reportAppError('MIDI','此瀏覽器不支援 Web MIDI API',{title:'MIDI 無法使用'});
    return;
  }
  try{
    ...
    setMidiStatus('MIDI：連接失敗','error');
    reportAppError('MIDI',err,{title:'MIDI 連接失敗'});
  }
}
...
async function startMicrophone(){
  try{
    ...
    reportAppError('麥克風',e,{title:'麥克風無法啟動'});
  }
}
function micLoop(){
...
window.addEventListener('resize',()=>{
  if($('practiceView').classList.contains('active')){
    renderStaticScore();
    setTimeout(()=>ensureScoreVisible('resize',true),80);
  }
});
